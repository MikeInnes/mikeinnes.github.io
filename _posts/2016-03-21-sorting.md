...
* Sorting, and the algotirthm used to do it efficiently
...
The target we set for summing a billion elements was 5 seconds. On my laptop, doing the sum actually takes around 15 seconds. The basic reason for this is reading chunks of the larger `BigVector` gets poor performance (around 750MiB/s) – much less than the initial benchmark. Strangely, performance is great for a 10<sup>8</sup> element vector, just not for 10<sup>9</sup> elements. To hazard a guess, it may be that the OS struggles with a large number of open file handles, although increasing the chunk size to around 1 GiB didn't seem to help.
...
*Julia is also participating in [Google Summer of Code](http://summerofcode.withgoogle.com) this year, so if you're excited about solving these kinds of problems you should check out our [ideas list](http://julialang.org/soc/ideas-page.html) and send in an application!*
