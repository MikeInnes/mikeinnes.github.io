...
"Any sufficiently advanced technology is indistinguishable from magic," go the famous words of Arthur C. Clarke. This seems particularly true in software, where we build enormous towers of abstraction over abstraction. It's truly incredible that the machinations of trillions of tiny bits and logic gates conspire to let millions around the globe idly beam cat pictures to each other.[^cats]

[^cats]: Perhaps for more than one reason.

Often a small dose of knowledge is enough to wash away our wide-eyed awe and bring us back to sweet, comfortable cynicism. Yet some ideas don't lose their lustre so easily; some abstractions are so powerful that it's a little hard to believe they work even after understanding them. Lazy lists are one such idea. Conveniently enough, they are also exactly the subject of this post.
...
[^haskell]: In fact, all data structures are lazy in Haskell; imagine doing this with trees as well.
...
Here's enough of a linked list to illustrate the ideas.
